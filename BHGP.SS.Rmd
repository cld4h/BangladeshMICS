---
title: SS Prior - Bayesian Hierarchical Generalized Poisson Model
---
...
ys <- dCSS(x,df_spike = 2, location_spike=0, scale_spike=2.8139628, df_slab = 2, location_slab=0, scale_slab = 0.02, ps=0.2)
yss <- dCSS(x,df_spike = 2, location_spike=0, scale_spike=0.23108825, df_slab = 2, location_slab=0, scale_slab = 0.02, ps=0.2)
...
plot(x, ys, type = "l", lty = 1, ylab = "PDF", xlab = "", cex.lab = 2, col = 1, ylim=c(0,20))
...
location_slab <- 0
p_spike <- 0.2
scale_slab <- 0.02 # This is spike, not slab

pmn.theta <- rep(0,p)
psd.theta <- NULL

# find the scale_spike that makes 95% coverage for [-6,6], (result=3.7411505)
# Comment out here to save time
...
# find the scale_spike that makes 95% coverage for [-1,1], (result=0.620573)
# Comment out here to save time
# for(i in 1:length(s)){
#   quant <- qCSS(0.975, df_spike, location_spike, s[i], df_slab, location_slab, scale_slab , p_spike)
#   if(quant > 1){
...
#psd.theta <- c(2.054671,rep(0.16963212,p-1))

#when ps=0.2
psd.theta <- c(3.7411505,rep(0.620573,p-1))

#when ps=0.3
# psd.theta <- c(2.8139628,rep(0.23108825,p-1))
```

```{r}
IMAGEOUT="./pics/BHGP.SS/"
x <- seq(-1, 1, by = 0.0001)
#ys1 <- dCSS(x, df_spike, location_spike, 0.16963212, df_slab, location_slab, scale_slab, p_spike)

#xcol<-gray.colors(4, start = 0, end = 0.8)
xcol<-c(1,2,3,4)
ys1 <- dCSS(x, df_spike, location_spike, 0.3057556, df_slab, location_slab, scale_slab, p_spike)

par(mar=c(4,4.5,1,1),mgp=c(1.5,-1,0))
plot(x, dnorm(x, mean=0, sd=0.51022), type="l", lwd = 3, col=xcol[1], ylim=c(0,15), ylab = "Prior densities", xlab = expression(paste(theta[k])), cex.lab = 2)
lines(x, dlaplace(x, mu=0, sigma=0.33381), lwd = 3, col=xcol[2])
lines(x, dcauchy(x, location = 0, scale = 0.07871), lwd = 3, col=xcol[3])
lines(x, ys1, type = "l", lwd = 3, cex.lab = 2, col=xcol[4])
...
legend(0.05, 12, c(
	...
	expression(paste("SS (",sigma[l], "=0.621,",sigma[s],"=0.020)"))),
	 lwd = c(3,3,3,3), col = xcol, cex = 1.5, bty = "n", xpd = TRUE)
dev.print(pdf, paste(IMAGEOUT, "Priors.Predictors.pdf",sep=""),width=10,height=10)
```
...
par(mar=c(4,4.5,1,1),mgp=c(1.5,-1,0))
plot(x, dnorm(x, mean=0, sd=3.06129), type="l", lwd = 3, col=xcol[1], ylab = "Prior densities", xlab = expression(theta[1]), ylim=c(0,15), cex.lab = 2)
lines(x, dlaplace(x, mu=0, sigma=2.00285), lwd = 3, col=xcol[2])
lines(x, dcauchy(x, location = 0, scale = 0.47222), lwd = 3, col=xcol[3])
lines(x, ys1, lwd = 3,  cex.lab = 2, col=xcol[4])
...
```{r eval=FALSE}
RESULTPATH = "./data_output/BHGP.SS/"
